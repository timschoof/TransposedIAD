function TransposedIADs(varargin)
% Run an experiment
...
CondCode=sprintf('%s-%ddBSPL-%03drms-%d',p.IAD,p.dBSPL,round(p.rms2use*1000),p.ModulationRate);
...
if ~p.trackAbsThreshold % the normal case
    fprintf(fout, 'listener,CondCode,date,time,trial,IAD,correct,order,response,step,rTime,rev');
...
while (num_turns<p.FINAL_TURNS  && limit<=p.MaxBumps && trial<(p.MAX_TRIALS-1))
    ...
    while ((num_correct < LEVITTS_CONSTANT(levitts_index)) && (num_wrong==0))
        ...
        if p.outputAllWavs
            % write out the trial
            % generate a string with SNR in dB
            currentSNRrounded = round(p.SNR_dB,1);
            currentSNRrounded=sprintf('%d_%1d', floor(currentSNRrounded),10*(currentSNRrounded-floor(currentSNRrounded)));
            audiowrite(fullfile(p.wavOutputDir,sprintf('T%02d-%s-o%d.wav',trial, currentSNRrounded,p.Order)),w,p.SampFreq);
            % write out 
            [wInQuiet, flag] = NoClipStereo(wInQuiet, 'stimuli in quiet');
            audiowrite(fullfile(p.wavOutputDir,sprintf('Qt%02d-%s-o%d.wav',trial,currentSNRrounded,p.Order)),wInQuiet,p.SampFreq);
            % write out the modulator
            [wUntransposed, flag] = NoClipStereo(wUntransposed, 'untransposed');
        ...
        %% stat rat section for output format, etc.
            ...
            if trial<=2 || (rand(1)<MyLogistic(p.SNR_dB,jnd,1,1/3))
                response=p.Order;
            else
                xx=(1:3);
                xx(p.Order)=[];
    ...
    %% For ILD only -- ensure SNR_dB is >= MIN_SNR_dB
    if (p.SNR_dB < p.MIN_SNR_dB) && strcmp(p.IAD, 'ILD')
        p.SNR_dB = p.MIN_SNR_dB;
    end
