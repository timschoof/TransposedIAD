function [w, wInQuiet, wUntransposed]=GenerateIADduple(p)
%
% based on GenerateIADtriple(p) - August 2018
%   
%

if p.Order==3 % not allowed!
    error('Order of stimuli cannot be 3 for 2I-2AFC presentation');
end
...
%     end
    ...
    % construct the 2 intervals with ISIs
    for i=1:2
        if strcmp(p.IAD, 'ITD')
            % function [w, untransposed]=GenerateITDpulse(ITDpresent, p)
            if p.MaximalDifference==0 % 
                [x, untransposed]=GenerateITDpulse(p.Order==i, p);
            else % give the listener the best chance - the typical case, perhaps
